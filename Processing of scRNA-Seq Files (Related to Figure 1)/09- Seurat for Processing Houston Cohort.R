...
data <- read.table("All_Houston_220826_Raw_Expression.txt"", sep="\t", head=TRUE, row.names=1)

Tumors.combined <- CreateSeuratObject(counts = data, project = "Houston", min.cells = 3, min.features = 200)
...
pdf("Houston_WT_Mutant_Tumors_QC_AF.pdf", height = 6, width = 20)
...
pdf("Houston_WT_Mutant_Tumors_ElbowPlot.pdf", height = 6, width = 6)
ElbowPlot(Tumors.combined, ndims=50)
dev.off()

write.table(Tumors.combined@meta.data, file="Houston_WT_Mutant_Tumors_Integrated_MetaData.txt", sep="\t", col.names=NA, quote=FALSE)


################# Louvain Clustering and UMAP Generation ###################


Tumors.combined <- RunUMAP(Tumors.combined, reduction = "pca", dims = 1:19)
Tumors.combined <- FindNeighbors(Tumors.combined, dims = 1:19)
Tumors.combined <- FindClusters(Tumors.combined, resolution = 0.3)


pdf("Houston_WT_Mutant_Tumors_Clusters_With_Labels.pdf", height= 6, width = 7)
DimPlot(Tumors.combined, reduction = "umap", label=TRUE)
dev.off()

pdf("Houston_WT_Mutant_Tumors_UMAP_Patient_ID.pdf", height= 6, width = 9)
...
saveRDS(Tumors.combined, file="Houston_Brain_Tumors.rds")
